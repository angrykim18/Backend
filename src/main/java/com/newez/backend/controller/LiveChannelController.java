...
public class LiveChannelController {
    ...
    // ✅ [수정] 웹 관리자 페이지용 API: 이제 서버 주소를 붙이지 않고 DB 원본 데이터를 그대로 반환합니다.
    ...
    // 안드로이드용 API는 기존과 동일하게 서버 주소를 붙여서 보내줍니다. (수정 없음)
    @GetMapping
    ...
    public LiveChannel createLiveChannel(@RequestBody LiveChannel liveChannel) {
        stripServerUrlFromStreamUrl(liveChannel);
    ...
    public ResponseEntity<LiveChannel> updateLiveChannel(@PathVariable Long id, @RequestBody LiveChannel channelDetails) {
        return liveChannelRepository.findById(id)
                .map(channel -> {
                    ...
                    stripServerUrlFromStreamUrl(channelDetails);
    ...
    private void stripServerUrlFromStreamUrl(LiveChannel channel) {
        List<Server> servers = serverRepository.findAll();
        if (!servers.isEmpty() && channel.getStreamUrl() != null) {
            String serverUrl = servers.get(0).getServerUrl();
            if (channel.getStreamUrl().startsWith(serverUrl)) {
                String cleanStreamUrl = channel.getStreamUrl().substring(serverUrl.length());
                channel.setStreamUrl(cleanStreamUrl);
            }
        }
    }
}